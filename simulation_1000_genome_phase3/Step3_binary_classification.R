##########################################
## 2025.12.15
## step3: qPRS risk classification
## binary traits
##########################################
...
ACC_fn <- function(pred, true){
  conf <- confusionMatrix(
    factor(pred),
    factor(true),
    positive = "1")
  return(c(conf$overall["Accuracy"],
...
pop_index <- apply(gaProp, 1, which.max)
...
binary_dat <- data.frame(Y = Y, G = pop_index)
  
## setting 1
## three groups having same baseline prevalence
binary_dat$beta0 <- -1.5
  
## setting 2
## Afr population having lower baseline prevalence
#binary_dat$beta0 <- ifelse(binary_dat$G == 2, rnorm(1, -2, 0.1), rnorm(1, -1.5, 0.1))

## setting 3
## Afr population having higher baseline prevalence
#binary_dat$beta0 <- ifelse(binary_dat$G == 2, rnorm(1, -1, 0.1), rnorm(1, -1.5, 0.1))
  
binary_dat$prob <- (exp(binary_dat$beta0 + 1*binary_dat$Y))/(1 + exp(binary_dat$beta0 + 1*binary_dat$Y))

binary_dat$L <- rbinom(n = N_indiv, size = 1, prob = binary_dat$prob)

## true prevalence (computed using all individuals)
## assume this value is known for evaluation analysis
tau <- 1 - mean(binary_dat$L)
...
y_dis <- binary_dat$L[idx_dis]
y_dis <- as.factor(y_dis)

y_tgt <- binary_dat$L[idx_tgt]
y_tgt <- as.factor(y_tgt)
...
if (use_pcs) {
  ...
  if (requireNamespace("irlba", quietly = TRUE)) {
    ...
    PCs_dis <- sv$u %*% diag(sv$d, nrow = n_pcs, ncol = n_pcs) 
    } else {
...
if (use_pcs) {
  pc_df <- as.data.frame(PCs_dis)
  colnames(pc_df) <- paste0("PC", seq_len(ncol(pc_df)))
  } else {
    pc_df <- NULL
    }
  
## fit logistic regression for a single SNP
fit_one_snp <- function(j) {
  SNP <- G_dis[, j]
  
  geno_vals <- unique(SNP[!is.na(SNP)])
  if (length(geno_vals) <= 1) {
    return(c(beta = NA, se = NA, z = NA, p = NA))
    }
  
  if (use_pcs) {
    df <- data.frame(y = y_dis, SNP = SNP, pc_df)
    } else {
      df <- data.frame(y = y_dis, SNP = SNP)
      }
  ...
  if (!("SNP" %in% rownames(co))) {
    return(c(beta = NA, se = NA, z = NA, p = NA))
    }
  
  c(beta = co["SNP", "Estimate"],
    se = co["SNP", "Std. Error"],
    z = co["SNP", "z value"],
    p = co["SNP", "Pr(>|z|)"]
  )
...
## loop over SNPs
res_mat <- t(sapply(seq_len(m), fit_one_snp))
res_dt  <- as.data.table(res_mat)
  
## update variant info with effect estimates
## log OR
sumstats <- data.table(
  rsid = val_dt$rsid,
  chrom = val_dt$chrom,
  pos = val_dt$pos,
  beta_hat = res_dt$beta, 
  se = res_dt$se,
  z = res_dt$z,
  p = res_dt$p,
...
global_pred_var <- data.frame(Target_gaProp, rPRS_2 = (global_EPRS$grPRS)^2)
  
## fit a Gamma regression with log link
fit <- glm(rPRS_2 ~ EUR + AFR,
           family = Gamma(link = "log"),
           control = glm.control(maxit = 10000),
           data = global_pred_var)
  
## predicted variance
global_pred_var$predicted <- predict(fit, type = "response")

global_qPRS <- data.frame(PRS = PRS_tgt, pred_mean = global_EPRS$gEPRS, pred_var = global_pred_var$predicted)
  
## compute global qPRS
global_qPRS$qPRS <- apply(global_qPRS[, c("PRS", "pred_mean", "pred_var")], 1, function(input) pnorm(input[1], mean = input[2], sd = sqrt(input[3]), lower = T))

global_qPRS_threshold <- quantile(global_qPRS$qPRS, tau)

global_qPRS_pred <- ifelse(global_qPRS$qPRS >= global_qPRS_threshold, 1, 0)

global_acc<- ACC_fn(global_qPRS_pred, y_tgt)
...
local_pred_var <- data.frame(Target_gaProp, rPRS_2 = (local_EPRS$lrPRS)^2)
...
## predicted variance
local_pred_var$predicted <- predict(fit, type = "response")
  
local_qPRS <- data.frame(PRS = PRS_tgt, pred_mean = local_EPRS$lEPRS, pred_var = local_pred_var$predicted)
  
## compute local qPRS
local_qPRS$qPRS <- apply(local_qPRS[, c("PRS", "pred_mean", "pred_var")], 1, function(input) pnorm(input[1], mean = input[2], sd = sqrt(input[3]), lower = T))

local_qPRS_threshold <- quantile(local_qPRS$qPRS, tau)

local_qPRS_pred <- ifelse(local_qPRS$qPRS >= local_qPRS_threshold, 1, 0)

local_acc <- ACC_fn(local_qPRS_pred, y_tgt)
...
## adj top 2 PCs
########################################
PC_adj_data <- data.frame(PRS = PRS_tgt, PCs_tgt_only[, 1:2])

PC_adj <- lm(PRS ~ ., data = PC_adj_data)
  
## predict mean
PC_adj_mean <- predict(PC_adj, type = "response")
PC_adj_PRS <- PC_adj$residuals
PC_adj_square <- PC_adj_PRS^2
PC_adj_pred_var_dat <- data.frame(PC_adj_square = PC_adj_square, PCs_tgt_only[, 1:2])
  

PC_adj_var_fit <- glm(PC_adj_square ~ ., family = Gamma(link = "log"), control = glm.control(maxit = 10000), data = PC_adj_pred_var_dat)

PC_pred_var <- predict(PC_adj_var_fit, type = "response")

PC_adj_qPRS <- data.frame(PRS = PRS_tgt, pred_mean = PC_adj_mean, pred_var = PC_pred_var)

PC_adj_qPRS$qPRS <- apply(PC_adj_qPRS[, c("PRS", "pred_mean", "pred_var")], 1, function(input) pnorm(input[1], mean = input[2], sd = sqrt(input[3]), lower = T))

PC_qPRS_threshold <- quantile(PC_adj_qPRS$qPRS, tau)

PC_qPRS_pred <- ifelse(PC_adj_qPRS$qPRS >= PC_qPRS_threshold, 1, 0)

PC2_acc <- ACC_fn(PC_qPRS_pred, y_tgt)
...
PC_adj_data <- data.frame(PRS = PRS_tgt, PCs_tgt_only[, 1:10])
PC_adj <- lm(PRS ~ ., data = PC_adj_data)
...
PC_adj_PRS <- PC_adj$residuals
PC_adj_square <- PC_adj_PRS^2
PC_adj_pred_var_dat <- data.frame(PC_adj_square = PC_adj_square, PCs_tgt_only[, 1:10])
...
PC10_acc <- ACC_fn(PC_qPRS_pred, y_tgt)
...
PC_adj_data <- data.frame(PRS = PRS_tgt, PCs_tgt_only[, 1:20])
PC_adj <- lm(PRS ~ ., data = PC_adj_data)
...
PC_adj_PRS <- PC_adj$residuals
PC_adj_square <- PC_adj_PRS^2
PC_adj_pred_var_dat <- data.frame(PC_adj_square = PC_adj_square, PCs_tgt_only[, 1:20])
...
PC20_acc <- ACC_fn(PC_qPRS_pred, y_tgt)


rbind(global_acc, local_acc, PC2_acc, PC10_acc, PC20_acc)
